...
with tab1:
    ...
    st.divider()
    st.subheader("🏆 Ranking por temporada (Elo)")
    seasons_tab1 = sorted(matches["date"].dt.year.unique(), reverse=True)
    season_tab1 = st.selectbox("Temporada", seasons_tab1, index=0, key="season_ranking")

    # pega o último rating de cada time DENTRO daquela temporada (snapshot de fim de ano)
    hist_season = elo_hist[elo_hist["date"].dt.year == season_tab1]
    ranking = (
        hist_season.sort_values("date")
        .groupby("team")
        .last()
        .reset_index()[["team", "rating"]]
        .rename(columns={"team": "Time", "rating": "Rating Elo"})
    ...
    st.caption("Rating no fim daquela temporada (carrega o histórico acumulado até ali, não reseta a cada ano).")
...
with tab3:
    ...
    st.caption("Escolha uma temporada — só aparecem times que disputaram aquele ano, pra evitar comparar rosters de épocas diferentes.")

    seasons = sorted(matches["date"].dt.year.unique(), reverse=True)
    season = st.selectbox("Temporada", seasons, index=0)

    season_matches = matches[matches["date"].dt.year == season]
    teams_list = sorted(set(season_matches["team_blue"]) | set(season_matches["team_red"]))
    ...
    if team_a == team_b:
        ...
        st.caption(
            f"⚠️ Probabilidade calculada com o rating **atual** (acumulado até a última partida do dataset), "
            f"não com o rating que cada time tinha especificamente em {season}."
        )
